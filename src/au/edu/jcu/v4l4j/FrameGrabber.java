 ...
 * {@link VideoDevice}. <code>FrameGrabber</code>s are not instantiated
 * directly. Instead, one of the <code>getXXXFrameGrabber()</code> methods must
 * be called on a {@link VideoDevice} to obtain one. The requested height and
 * width may be adjusted to the closest supported values. The adjusted width and
 * height can be retrieved by calling {@link #getWidth()} and
 * {@link #getHeight()}.<br>
 * Frame grabbers operate in push mode: you must give v4l4j an object
 * implementing the {@link CaptureCallback} interface by calling
 * {@link #setCaptureCallback(CaptureCallback)}. During capture, v4l4j will
 * create a thread and deliver new frames to the capture callback object. By
 * default, new threads are created by the thread factory returned by
 * <code>Executors.defaultThreadFactory()</code>, but you can provide your own
 ...
 * A typical <code>FrameGrabber</code> use case is as follows: <br>
 * Create the video device and frame grabber: <code><br><br>
 ...
 * </code>
 * 
 * For a concrete example of push mode capture which displays the video stream,
 * see the au.edu.jcu.v4l4j.examples.SimpleViewer example application. <br>
 * Again, you must recycle video frames when they are no longer used.<br>
 * </br>
 * 
...
public interface FrameGrabber {
	 ...
	 * The returned format specifies the image format in which frames are
	 ...
	 * driver. The driver stores captured frames in these buffers and returns
	 ...
	 * one of the previous frames to be recycled through
	 * {@link VideoFrame#recycle()}.<br>
	 * You can specify how many buffers the driver should use by setting the
	 * <code>v4l4j.num_driver_buffers</code> property to an integer value
	 * <b>before creating a frame grabber object</b>. The number you specify is
	 * only an indication, and the driver can decide to allocate a different
	 * number of buffers. Before setting this number, make sure you fully
	 ...
	 * store new incoming frames. This number indicates how many more frames can
	 * be captured by v4l4j before the capture stops until you recycle one of
	 * the previous frame that was delivered to your application by calling
	 * {@link VideoFrame#recycle()}.
	 * 
	 ...
	 * accepted. {@link ResolutionInfo} objects can be obtained for each
	 * {@link ImageFormat}. See the {@link ImageFormat} and
	 * {@link ResolutionInfo} documentation for more information.
	 * 
	 * @param num
	 *            the numerator of the frame interval to be set
	 * @param denom
	 ...
	 *             if setting the frame interval is not supported.
	 * @throws StateException
	 *             if capture is ongoing (the frame interval must be queried
	 *             when not capturing), or if this <code>FrameGrabber</code> has
	 ...
	 *             if the selected input does not have a tuner
	 * @throws StateException
	 *             if this <code>FrameGrabber</code> has been already released,
	 *             and therefore must not be used anymore.
	 */
	 ...
	 *             if no valid {@link CaptureCallback} object was provided (call
	 *             {@link #setCaptureCallback(CaptureCallback callback)} OR if
	 *             the capture cannot be started
	 * @throws StateException
	 *             if this <code>FrameGrabber</code> has been already released,
	 *             and therefore must not be used anymore
	 */
