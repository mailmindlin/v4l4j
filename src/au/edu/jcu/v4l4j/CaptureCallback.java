 ...
 * when new captured frames become available, or an exception occurs during a
 * capture. In order to capture from a video device, you must instantiate an
 * object which implements this interface and pass it to a frame grabber
 * (via @link {@link FrameGrabber#setCaptureCallback(CaptureCallback)}). When
 * you start the capture, v4l4j will call {@link #nextFrame(VideoFrame)} to
 * deliver new frames to your application as soon as they arrive. Check the
 * {@link FrameGrabber} page for more information.
 * 
 * @author gilles
 *
 */
