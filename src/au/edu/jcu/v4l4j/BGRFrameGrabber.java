 ...
 * {@link VideoDevice}. A BGR24 frame grabber can only be created if the
 * associated video device can produce images in a format v4l4j knows how to
 * encode in BGR24. The {@link VideoDevice#supportBGRConversion()} method can be
 * used to find out whether a video device can have its images BGR24-encoded by
 * v4l4j, ie if a BGR24 frame grabber can be instantiated.
 * <code>BGRFrameGrabber</code> objects are not instantiated directly. Instead,
 * the {@link VideoDevice#getBGRFrameGrabber(int, int, int, int)} or
 * {@link VideoDevice#getBGRFrameGrabber(int, int, int, int, ImageFormat)}
 * method must be called on the associated {@link VideoDevice}. BGR frame
...
public class BGRFrameGrabber extends AbstractGrabber {
	 ...
	 * 
	 * @param di
	 *            the DeviceInfo of the video device
	 * @param o
	 ...
	 *            thread.
	 */
	BGRFrameGrabber(DeviceInfo di, long o, int w, int h, int ch, int std, Tuner t, ImageFormat imf,
	 ...
	 *             BGR24 encoded. If no image format was chosen, the video
	 *             device does not have any image formats that can be BGR24
	 ...
	 * one images are retrieved from the device BEFORE BGR conversion.
	 * 
