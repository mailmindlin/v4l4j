 ...
 * {@link VideoDevice#getRawFrameGrabber(int, int, int, int)
 * getRawFrameGrabber()} method must be called on the associated
 * {@link VideoDevice}. Raw frame grabbers implement the {@link FrameGrabber}
 * interface which provides methods to handle video capture. See
 * {@link FrameGrabber its documentation} for more information.
 * 
...
public class RawFrameGrabber extends AbstractGrabber {
	/**
	 * This constructor builds a raw FrameGrabber object used to raw frames from
	 * a video source.
	 * 
	 * @param di
	 ...
	 *            grabber
	 * @param o
	 *            a JNI pointer to the v4l4j_device structure
	 * @param w
	 *            the requested frame width
	 * @param h
	 *            the requested frame height
	 * @param ch
	 *            the input index, as returned by
	 ...
	 *            V4L4JConstants)
	 * @param t
	 *            the {@link Tuner} associated with this frame grabber or
	 *            <code>null</code>.
	 * @param imf
	 *            the image format frames should be captured in
	 * @param factory
	 *            the thread factory to sue when creating the push source
	 * @throws ImageFormatException
	 *             if the image format is null and a RAW frame grabber is to be
	 *             created
	 */
	RawFrameGrabber(DeviceInfo di, long o, int w, int h, int ch, int std, Tuner t, ImageFormat imf,
			ThreadFactory factory) throws ImageFormatException {
		super(di, o, w, h, ch, std, t, imf, RAW_GRABBER, factory);
	}

	/**
	 * This method returns the native image format used by this FrameGrabber.
	 * The returned format specifies the image format in which frames captured
	 * by this grabber are.
	 * 
