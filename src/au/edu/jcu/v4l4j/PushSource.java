...
/**
 * PushSource instances create their own thread which polls a frame grabber and
 * notify the {@link CaptureCallback} object given in the constructor each time
...
class PushSource implements Runnable {
	...
	/**
	 * This method builds a new <code>PushSource</code> instance which will
	 * obtain frames from the given frame grabber and pass them to the given
	 * callback object.
	 * 
	 * @param grabber
	 ...
	 * 
	 * @throws StateException
	 *             if the capture has already been stopped
	 */
			...
			if (!Thread.currentThread().equals(thread)) {
				while (state == STATE_ABOUT_TO_STOP) {
					try {
						...
						// frame pusher thread to complete");
						// e.printStackTrace();
						// throw new StateException("interrupted while waiting
						// for frame pusher thread to complete", e);
					}
				}
			}
	...
	public final void run() {
		...
		while (!Thread.interrupted()) {
			try {
				...
				// (ie. it does not
				// happen as the result of the capture having been stopped or
				// the frame
