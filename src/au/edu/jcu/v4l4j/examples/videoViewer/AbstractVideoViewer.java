...
/**
 * This class builds a GUI to display a video stream and video controls from a
 * {@link VideoDevice}.
 * 
 * @author gilles
 *
...
public abstract class AbstractVideoViewer extends WindowAdapter implements CaptureCallback {
	 ...
	 * 
	 * @param i
	 *            an array of {@link ImageFormat}s to be added in the format
	 *            list
	 * @param fmtName
	 *            the name of format of images displayed in the title bar. If
	 *            this array is empty, the format list is disabled and capture
	 *            cannot be started
	 * @param width
	 *            the capture width
	 * @param height
	...
	/**
	 * This abstract method must be implemented by subclasses and return and
	 * frame grabber which captures frames in the given native image format
	 * 
	 * @param imf
	 *            the native image format the returned frame grabber must use
	 * @return a frame grabber
	 * @throws V4L4JException
	 *             if a frame grabber cant be created
	 */
	abstract protected FrameGrabber getFrameGrabber(ImageFormat imf) throws V4L4JException;

	/**
	 * This method is called when the Start capture button is hit. It creates a
	 * frame grabber, resizes the UI and start the capture
	 */
	private void startCapture() {
		...
		try {
			...
			freqSpinner.setModel(new SpinnerNumberModel(new Double(tuner.getFrequency()),
					new Double(tinfo.getRangeLow()), new Double(tinfo.getRangeHigh()), new Double(1)));
	...
	/**
	 * This method is called when the Stop capture button is hit. It terminates
	 * the capture, releases resources and reset the UI
	 */
	...
	/**
	 * this method is called by v4l4j during capture when a new frame is
	 * available
	 */
	@Override
	...
	public class ControlModelGUI implements ControlGUI {
		...
		private void initControlGUI() {
			...
			TitledBorder b = BorderFactory.createTitledBorder(BorderFactory.createEtchedBorder(EtchedBorder.LOWERED),
					ctrl.getName());
	...
	public class LongControl extends ControlModelGUI implements ChangeListener {
		...
		public LongControl(Control c) {
			super(c);
			spinner = new JSpinner(new SpinnerNumberModel(new Long(0), Long.valueOf(Long.MIN_VALUE),
					Long.valueOf(Long.MAX_VALUE), new Long(1)));
	...
	public class TextControl extends ControlModelGUI implements ActionListener {
		...
		public TextControl(Control c) {
			...
			try {
				textField.setText(c.getStringValue());
			} catch (V4L4JException e) {
			}
		...
		public void actionPerformed(ActionEvent evt) {
			String value = "";
			try {
				value = ctrl.setStringValue(textField.getText());
			} catch (Exception e) {
				JOptionPane.showMessageDialog(null, "Error setting value.\n" + e.getMessage());
				try {
