...
/**
 * Objects implementing this interface represent a single video frame captured
 * by v4l4j. They provide access to image data through various objects,
 * including byte arrays and {@link DataBuffer}s. During capture, v4l4j creates
 ...
 * <code>byte[].length</code>.<br>
 * From the moment a <code>VideoFrame</code> is returned by a frame grabber, the
 * <code>VideoFrame</code> object itself as well as any of the objects obtained
 * through its methods can be used. <b>When the <code>VideoFrame</code> and any
 * of the objects obtained through it) are dealt with and no longer used, the
 * <code>VideoFrame</code> must be recycled, which marks it as being available
 * to v4l4j to store an upcoming image.</b> From the moment a
 * <code>VideoFrame</code> is recycled, neither the <code>VideoFrame</code> nor
 * objects obtained through it can be used. Bad things will happen if you
 * violate this rule. It is recommended that you explicitly set all references
 * to this object, as well as references to objects obtained through this frame,
 * to null.<br>
 * Note that not all video frames are capable of returning a raster or a
 * buffered image. For example, rasters cannot be created for compressed image
 * formats, which means <code>VideoFrame</code>s returned by
 * {@link JPEGFrameGrabber}s cannot return a buffered image, and calling
 * {@link #getBufferedImage()} will throw a {@link UnsupportedMethod} exception.
 * In the same way, video frames returned by raw frame grabbers cannot create
 * rasters and an {@link UnsupportedMethod} exception will be thrown if
 * {@link #getRaster()} is called.
 * 
 * @author gilles
 *
 */
public interface VideoFrame {
	 ...
	 * find out when a frame was dropped: if <code>currentSequenceNumber != 
	 ...
	 * 
	 * @return the OS time (number of microseconds elapsed since startup) at
	 *         which this video frame was captured.
	 * @throws StateException
	 *             if this video frame has been recycled already.
	 */
	long getCaptureTime();
	 ...
	 * 
	 * @return the image data as a byte array. The array may be longer than the
	 *         actual frame length.
	 * @throws StateException
	 *             if this video frame has been recycled already.
	 */
	 ...
	 * compressed formats like JPEG). In this case this method will throw a
	 ...
	 * image formats (including raw image formats). In this case this method
