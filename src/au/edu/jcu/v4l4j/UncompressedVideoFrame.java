...
/**
 * This class of object encapsulate data for a video frame in an uncompressed
 * image format. Image using a format that can be stored in a {@link Raster}
 * must pass a {@link SampleModel} to the constructor. Image using a format that
 * can be encapsulated in a {@link BufferedImage} must pass a {@link ColorSpace}
 * to the constructor.
 * 
 * @author gilles
 *
...
class UncompressedVideoFrame extends BaseVideoFrame {
	 ...
	 * 
	 * @param grabber
	 *            the {@link FrameGrabber} to which this frame must be returned
	 *            to when recycled
	 * @param bufferSize
	 *            the size in bytes of the byte array to be created
	 * @param sm
	 *            the SampleModel used to build a WritableRaster or null if no
