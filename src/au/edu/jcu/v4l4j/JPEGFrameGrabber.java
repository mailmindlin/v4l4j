 ...
 * {@link VideoDevice}. A JPEG frame grabber can only be created if the
 * associated video device can produce images in a format v4l4j knows how to
 * encode in JPEG. The {@link VideoDevice#supportJPEGConversion()} method can be
 * used to find out whether a video device can have its images JPEG-encoded by
 * v4l4j, ie if a JPEG frame grabber can be instantiated.
 * <code>JPEGFrameGrabber</code> objects are not instantiated directly. Instead,
 * the {@link VideoDevice#getJPEGFrameGrabber(int, int, int, int, int)} or
 * {@link VideoDevice#getJPEGFrameGrabber(int, int, int, int, int, ImageFormat)}
 * method must be called on the associated {@link VideoDevice}. JPEG frame
...
public class JPEGFrameGrabber extends AbstractGrabber {
	 ...
	 *             JPEG encoded. If no image format was chosen, the video device
	 *             does not have any image formats that can be jpeg encoded (let
	 *             the author know, see README file)
	 ...
	 * 
	 * @param q
	 *            the quality (between {@link V4L4JConstants#MIN_JPEG_QUALITY}
	 *            and {@link V4L4JConstants#MAX_JPEG_QUALITY} inclusive)
	 * @throws StateException
	 ...
	 * one images are retrieved from the device BEFORE JPEG conversion.
	 * 
