...
/**
 * Instances of this class are used to represent a tuner attached to a video
 * input. Methods in this class control the frequency of the tuner and report
 ...
 * high). The AFC may not be available on all hardware. If not,
 * {@link #getAFC()} returns a value of 0.<code>Tuner</code>s are not directly
 * instantiated. Instead, to retrieve the <code>Tuner</code> associated with a
 * frame grabber, call {@link FrameGrabber#getTuner()} on the associated
 * {@link FrameGrabber} object. Alternatively, you can also obtain a list of
 * {@link Tuner} objects by calling {@link VideoDevice#getTunerList()}. Methods
 * in this class must not be called once the associated video device has been
 * released. Otherwise, a {@link StateException} will be thrown.
 * 
 * @author gilles
 *
 */
public class Tuner {
	 ...
	 *             if the associated frame grabber has been released and this
	 *             tuner must not be used anymore.
	 */
	 ...
	 * When its value is negative, the frequency is too low and when positive,
	 * it is too high.
	 * 
