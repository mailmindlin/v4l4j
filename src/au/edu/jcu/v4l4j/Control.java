 ...
 * <code>Control</code> objects are not directly instantiated. Instead a list of
 * <code>Control</code>s associated with a <code>VideoDevice</code> can be
 * obtained by calling {@link VideoDevice#getControlList() getControlList()}.
 * Once the controls are no longer used, they must be released by calling
 * {@link VideoDevice#releaseControlList() releaseControlList()}. Any attempt to
 * use a control after it has been released will result in a
 * {@link StateException}.<br>
 * A control can be either of the following types:<br>
 * <ul>
 * <li><code>V4L4JConstants.CTRL_TYPE_BUTTON</code>: Such controls take only 2
 * different values: 0 and 1</li>
 * <li><code>V4L4JConstants.CTRL_TYPE_SLIDER</code>: Controls of this type take
 * a range of values between a minimum value (returned by {@link #getMinValue()}
 * ) and a maximum value (returned by {@link #getMaxValue()}) with an increment
 * value (returned by {@link #getStepValue()})</li>
 * <li><code>V4L4JConstants.CTRL_TYPE_SWITCH</code>: Switch controls do not take
 * any specific value, and attempting to read its current value will always
 * return 0. Setting any value will activate the switch.</li>
 * <li><code>V4L4JConstants.CTRL_TYPE_DISCRETE</code>: These controls accept
 * only specific discrete values, which can be retrieved using
 * {@link #getDiscreteValues()}. Each discrete value may be accompanied by a
 * description, which is returned by {@link #getDiscreteValueNames()}.</li>
 * <li><code>V4L4JConstants.CTRL_TYPE_STRING</code>: These controls accept
 * string values, which can be retrieved using {@link #getStringValue()} and set
 * with {@link #setStringValue(String)}. Calls to any other get/set methods will
 * result in {@link UnsupportedMethod} exceptions. {@link #getMinValue()} &
 * {@link #getMaxValue()} return the smallest / largest string length (number of
 * characters) this control will accept. {@link #getStepValue()} specify the
 * step length of the string.</li>
 * <li><code>V4L4JConstants.CTRL_TYPE_LONG</code>: These controls accept long
 * values, between {@link Long#MIN_VALUE} and {@link Long#MAX_VALUE} with a step
 * value of 1. The actual value is set / retrieved with
 * {@link Control#setLongValue(long)} and {@link Control#getLongValue()}. Calls
 * to any other get/set methods, {@link #getMinValue()}, {@link #getMaxValue()}
 * and {@link #getStepValue()} will result in {@link UnsupportedMethod}
 * exceptions.</li>
 * <li><code>V4L4JConstants.CTRL_TYPE_BITMASK</code>: These controls accept
 * bitmask values, between 0 and {@link Integer#MAX_VALUE} with a step value of
 * 1. The actual value is set / retrieved with {@link Control#setValue(int)} and
 * {@link Control#getValue()}. Calls to any other get/set methods will result in
 * {@link UnsupportedMethod} exceptions.</li>
 * </ul>
 * 
 * @author gilles
 *
...
public class Control {
	 ...
	 * @param min
	 *            the minimum value it will accept
	 * @param max
	 *            the maximum value it will accept
	 * @param step
	 *            the increments
	 * @param type
	 *            the type of this control
	 * @param names
	 *            the names of the discrete values (if any), otherwise null
	 * @param values
	 *            the discrete values if any. Otherwise null
	 * @param o
	 *            A C pointer to a struct v4l4j_device
	 */
	 ...
	 *             {@link V4L4JConstants#CTRL_TYPE_STRING} (and you should be
	 *             using {@link #getStringValue()} instead) or if this control
	 *             is of type {@link V4L4JConstants#CTRL_TYPE_LONG} (and you
	 *             should be using {@link #setLongValue(long)} instead).
	...
	/**
	 * This method sets a new value for this control. The returned value is the
	 * new value of the control, which for normal controls will be identical to
	 * the one set. However, some controls (for example relative values like pan
	 * or tilt) are write-only and getting their value does not make sense. The
	 * return value in this case is the control's default value (as returned by
	 * {@link #getDefaultValue()}).
	 * 
	 * @param value
	 *            the new value
	 * @return the new value of the control after setting it, or the control's
	 *         default value
	 * @throws ControlException
	 ...
	 *             if this control is of type
	 *             {@link V4L4JConstants#CTRL_TYPE_STRING} (and you should be
	 *             using {@link #setStringValue(String)} instead) or if this
	 *             control is of type {@link V4L4JConstants#CTRL_TYPE_LONG} (and
	 *             you should be using {@link #setLongValue(long)} instead).
	 * @throws StateException
	 *             if this control has been released and must not be used
	 *             anymore.
	 */
	...
	/**
	 * This method retrieves the current string value of this control. Some
	 * controls may be write-only and getting their value does not make sense.
	 * Invoking this method on this kind of controls will trigger a
	 * ControlException.
	 * 
	 ...
	 *             if the value cannot be retrieved
	 * @throws UnsupportedMethod
	 *             if this control is not of type
	 *             {@link V4L4JConstants#CTRL_TYPE_STRING}.
	 * @throws StateException
	 *             if this control has been released and must not be used
	 *             anymore.
	 */
	 ...
	 * @throws ControlException
	 *             if the value can not be set, or if the new string value's
	 *             length is under / over the minimum / maximum length.
	 * @throws UnsupportedMethod
	 *             if this control is not of type
	 *             {@link V4L4JConstants#CTRL_TYPE_STRING},
	 * @throws StateException
	...
	/**
	 * This method retrieves the current long value of this control. Some
	 * controls may be write-only and getting their value does not make sense.
	 * Invoking this method on this kind of controls will trigger a
	 * ControlException.
	 * 
	 ...
	 * @throws UnsupportedMethod
	 *             if this control is not of type
	 *             {@link V4L4JConstants#CTRL_TYPE_LONG}.
	 * @throws StateException
	 *             if this control has been released and must not be used
	 *             anymore.
	 */
	...
	/**
	 * This method sets a new long value for this control. The returned value is
	...
	/**
	 * This method increases this control's current value by its step (as
	 * returned by {@link #getStepValue()})</code>. The returned value is the
	 * new value of the control, which for normal controls will be identical to
	 * the old one plus the step value. However, some controls (for example
	 * relative values like pan or tilt) are write-only and getting their value
	 * does not make sense. In this case, this method sets the new value to the
	 * default one plus the step value. Note that, the return value in this case
	 * is still the control's default value (as returned by
	 * {@link #getDefaultValue()}), not the default one plus the step value.
	 * 
	 * @return the new value of the control after increasing its old value, or
	 *         the control's default value
	 * @throws ControlException
	 *             if the value cannot be increased
	 * @throws UnsupportedMethod
	 *             if this control is of type
	 *             {@link V4L4JConstants#CTRL_TYPE_STRING} or
	 *             {@link V4L4JConstants#CTRL_TYPE_LONG}
	 * @throws StateException
	 *             if this control has been released and must not be used
	 *             anymore.
	...
	public int increaseValue() throws ControlException {
		...
		if (type == V4L4JConstants.CTRL_TYPE_STRING)
			throw new UnsupportedMethod(
					"This control is a string control and does not accept calls to increaseValue()");
		if (type == V4L4JConstants.CTRL_TYPE_LONG)
			throw new UnsupportedMethod("This control is a long control and does not support calls to increaseValue()");

		/**
		 * the following try statement is here so that write-only controls
		 * (Relative Pan for instance) that return a ControlException when read
		 * can still have their value increase()d.
		 */
		state.get();
		try {
	...
	/**
	 * This method decreases this control's current value by its step (as
	 * returned by {@link #getStepValue()}. The returned value is the new value
	 * of the control, which for normal controls will be identical to the old
	 * one minus the step value. However, some controls (for example relative
	 * values like pan or tilt) are write-only and getting their value does not
	 * make sense. In this case, this method sets the new value to the default
	 * one minus the step value. Note that, the return value in this case is
	 * still the control's default value (as returned by
	 * {@link #getDefaultValue()}), not the default one plus the step value.
	 * 
	 * @return the new value of the control after decreasing it.
	 * @throws ControlException
	 *             if the value can not be increased
	 * @throws UnsupportedMethod
	 *             if this control is of type
	 *             {@link V4L4JConstants#CTRL_TYPE_STRING} or
	 *             {@link V4L4JConstants#CTRL_TYPE_LONG}
	...
	public int decreaseValue() throws ControlException {
		...
		if (type == V4L4JConstants.CTRL_TYPE_STRING)
			throw new UnsupportedMethod(
					"This control is a string control and does not support calls to decreaseValue()");
		if (type == V4L4JConstants.CTRL_TYPE_LONG)
		 ...
		 * the following try statement is here so that write-only controls
		 * (Relative Pan for instance) that return a ControlException when read
		 * can still have their value decrease()d
		 */
		state.get();
		try {
			old = doGetValue(this.object, id);
	...
	/**
	 * This method retrieves the maximum value this control will accept. If this
	 * control is a string control, this method returns the maximum string size
	 * that can be set on this control.
	 * 
	...
	/**
	 * This method retrieves the minimum value this control will accept. If this
	 * control is a string control, this method returns the minimum string size
	 * that can be set on this control.
	 * 
	 ...
	 * for this control. New values must be equal to
	 * <code>getMin() + K*getStep()</code> where K is an integer, and the result
	 * is less or equal to {@link #getMaxValue()}. For string control, this
	 * method returns the step size of the string.
	 * 
	 ...
	 * <code>V4L4JConstants.CTRL_TYPE_BUTTON</code>,
	 * <code>V4L4JConstants.CTRL_TYPE_SLIDER</code>,
	 * <code>V4L4JConstants.CTRL_TYPE_SWITCH</code>,
	 * <code>V4L4JConstants.CTRL_TYPE_DISCRETE</code>,
	 * <code>V4L4JConstants.CTRL_TYPE_STRING</code>,
	 * <code>V4L4JConstants.CTRL_TYPE_LONG</code>,
	...
	/**
	 * This method returns a list of the discrete values accepted by this
	 * control. The list is ordered in the same way as the list returned by
	 * {@link #getDiscreteValueNames()}. So a value at index 'i' in the list
	 * returned by this method, matches the name at the same index in the list
	 * returned by {@link #getDiscreteValueNames()}.
	 * 
	...
	/**
	 * This method returns the list of names for each of the supported discrete
	 * values. The list is ordered in the same way as the list returned by
	 * {@link #getDiscreteValues()}. So the name at index 'i' in the list
	 * returned by this method, matches the value at the same index in the list
	 * returned by {@link #getDiscreteValues()}.
	 * 
