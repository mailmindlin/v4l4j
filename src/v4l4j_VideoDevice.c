 ...
 */
JNIEXPORT jlong JNICALL Java_au_edu_jcu_v4l4j_VideoDevice_doInit(JNIEnv *env, jclass me, jstring dev) {
	LOG_FN_ENTER();
	struct v4l4j_device *device;
	XMALLOC(device, struct v4l4j_device *, sizeof(struct v4l4j_device));
	
	const char * device_file = (*env)->GetStringUTFChars(env, dev, 0);
	dprint(LOG_LIBVIDEO, "[LIBVIDEO] Calling open_device()\n");
	device->vdev = open_device((char *)device_file);
	(*env)->ReleaseStringUTFChars(env, dev, device_file);
	
	if(device->vdev == NULL) {
		THROW_EXCEPTION(env, GENERIC_EXCP, "Error creating new VideoDevice object");
		return 0;
	}
 ...
 */
JNIEXPORT void JNICALL Java_au_edu_jcu_v4l4j_VideoDevice_doRelease(JNIEnv *e, jclass me, jlong object) {
	LOG_FN_ENTER();
	struct v4l4j_device *device = (struct v4l4j_device *) (uintptr_t) object;
	int ret;

	if ((ret = close_device(device->vdev)) != 0 ) {
...
static jintArray get_values(JNIEnv *env, struct control *l) {
	const int len = l->count_menu;
	
	jintArray values_array = (*env)->NewIntArray(env, len);
	if(!values_array) {
		THROW_EXCEPTION(env, JNI_EXCP, "Error creating the values array");
		return NULL;
	}
	...
	if (!values) {
		THROW_EXCEPTION(env, JNI_EXCP, "Error allocating temporary values array");
		return NULL;
	}
	...
	(*env)->SetIntArrayRegion(env, values_array, 0, l->count_menu, values);
	XFREE(values);
...
static jobjectArray get_names(JNIEnv *env, struct control *l) {
	jclass string_class = (*env)->FindClass(env, "java/lang/String");
	if(!string_class) {
		THROW_EXCEPTION(env, JNI_EXCP, "Error up class java.lang.String");
		return NULL;
	}
	
	jobjectArray names_array = (*env)->NewObjectArray(env, l->count_menu, string_class, NULL);
	(*env)->DeleteLocalRef(env, string_class);
	if(!names_array) {
		info("[V4L4J] Error creating the names array\n");
		THROW_EXCEPTION(env, JNI_EXCP, "Error creating the name array");
		return NULL;
	}
	...
	for(unsigned int i = 0; i < l->count_menu; i++) {
		...
		if(l->v4l2_ctrl->type == V4L2_CTRL_TYPE_MENU) {
			...
			THROW_EXCEPTION(env, JNI_EXCP, "Unknown menu control type\n");
			return NULL;
		}
		jstring nameStr = (*env)->NewStringUTF(env, name);
		(*env)->SetObjectArrayElement(env, names_array, i, nameStr);
 ...
 */
JNIEXPORT void JNICALL Java_au_edu_jcu_v4l4j_VideoDevice_doReleaseControlList(JNIEnv *e, jclass me, jlong object){
	LOG_FN_ENTER();
	struct v4l4j_device *device = (struct v4l4j_device *) (uintptr_t) object;
	
	dprint(LOG_LIBVIDEO, "[LIBVIDEO] Calling release_control_list()\n");
	release_control_list(device->vdev);
}

JNIEXPORT void JNICALL Java_au_edu_jcu_v4l4j_VideoDevice_doGetTunerActions(JNIEnv *e, jclass me, jlong o){
	LOG_FN_ENTER();
	struct v4l4j_device *device = (struct v4l4j_device *) (uintptr_t) o;
	
	dprint(LOG_LIBVIDEO, "[LIBVIDEO] Calling get_tuner_actions()\n");
	get_tuner_actions(device->vdev);
}

JNIEXPORT void JNICALL Java_au_edu_jcu_v4l4j_VideoDevice_doReleaseTunerActions(JNIEnv *e, jclass me, jlong o){
	LOG_FN_ENTER();
	struct v4l4j_device *device = (struct v4l4j_device *) (uintptr_t) o;
	
	dprint(LOG_LIBVIDEO, "[LIBVIDEO] Calling release_tuner_actions()\n");
	release_tuner_actions(device->vdev);
}
