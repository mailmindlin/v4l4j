...
static void fix_quirky_values(struct video_device *vdev, unsigned int idx, unsigned int *f) {
	...
	if(*f < t.rangelow) {
		...
		*f = t.rangelow;
		...
		dprint(LIBVIDEO_SOURCE_TUNER, LIBVIDEO_LOG_DEBUG, "TUN: QUIRKS: Tuner frequency %u for tuner %d on device %s ABOVE max %u\n", *f, idx, vdev->file, t.rangehigh);
		*f = t.rangehigh;
		return;
	}
}
...
int set_tuner_freq_v4l2(struct video_device *vdev, unsigned int idx, unsigned int f) {
	...
	dprint(LIBVIDEO_SOURCE_TUNER, LIBVIDEO_LOG_ERR, "TUN: Setting frequency to %u for tuner %d on device %s\n", f, idx, vdev->file);

	freq.tuner = idx;
	if(ioctl(vdev->fd, VIDIOC_G_FREQUENCY, &freq) != 0) {
...
int get_tuner_freq_v4l2(struct video_device *vdev, unsigned int idx, unsigned int *f) {
	...
	freq.tuner = idx;
	if(ioctl(vdev->fd, VIDIOC_G_FREQUENCY, &freq) != 0) {
		dprint(LIBVIDEO_SOURCE_TUNER, LIBVIDEO_LOG_ERR, "TUN: Failed to get tuner frequency for tuner %d on device %s\n", idx, vdev->file);
		return LIBVIDEO_ERR_IOCTL;
	}
	*f = freq.frequency;
	fix_quirky_values(vdev, idx, f);
	dprint(LIBVIDEO_SOURCE_TUNER, LIBVIDEO_LOG_ERR, "TUN: Got frequency %u for tuner %d on device %s\n", *f, idx, vdev->file);
	return LIBVIDEO_ERR_SUCCESS;
}
...
int get_rssi_afc_v4l2(struct video_device *vdev, unsigned int idx, int *r, int *a) {
	...
	if(ioctl(vdev->fd, VIDIOC_G_TUNER, &t) != 0) {
		dprint(LIBVIDEO_SOURCE_TUNER, LIBVIDEO_LOG_ERR, "TUN: Failed to get tuner info for tuner %d on device %s\n", idx, vdev->file);
		return LIBVIDEO_ERR_IOCTL;
	}
	dprint(LIBVIDEO_SOURCE_TUNER, LIBVIDEO_LOG_DEBUG, "TUN: Got RSSI %d & AFC %d for tuner %d on device %s\n", t.signal, t.afc, idx, vdev->file);
	*r = t.signal;
	*a = t.afc;
	return LIBVIDEO_ERR_SUCCESS;
}
