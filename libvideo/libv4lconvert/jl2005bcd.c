...
int v4lconvert_decode_jl2005bcd(struct v4lconvert_data *data, const u8 *src, unsigned int src_size, u8 *dest, u32 width, u32 height) {
	...
	if (height != (unsigned int) src[4] << 3) {
		V4LCONVERT_ERR("Height is %d, not %d\n", src[4] << 3, height);
		return 1;
	}

	if (width != (unsigned int) src[5] << 3) {
	...
	if (q <= 0)
		...
		scale_factor = 2 * (100 - q);
	else
		scale_factor = 0;
	jpeg_set_linear_quality(&cinfo, scale_factor, TRUE);
	...
	src += 16;
	unsigned int jpeg_data_size = src_size - 16;

	unsigned int jpeg_data_idx = 0;


	dinfo.err = jpeg_std_error (&jderr);
	jpeg_create_decompress (&dinfo);
	for (unsigned int x = 0; x < width; x += 16) {
		int eoi = find_eoi(data, src, jpeg_data_idx, jpeg_data_size);
		if (eoi < 0)
			return eoi;

		unsigned int size = (unsigned) eoi - jpeg_data_idx;
		if ((JPEG_HEADER_SIZE + size) > sizeof(jpeg_stripe)) {
			V4LCONVERT_ERR("stripe size too big %d > %zu\n",
		...
		for (unsigned int y = 0; y < height; y += 16) {
			...
			for (unsigned int y1 = 0; y1 < 16; y1 += 2) {
				for (unsigned int x1 = 0; x1 < 16; x1 += 2) {
