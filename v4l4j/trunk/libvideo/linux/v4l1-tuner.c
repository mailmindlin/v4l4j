...
static void fix_quirky_values(struct video_device *vdev, int idx, unsigned int *f){
	...
	if(-1 == ioctl(vdev->fd, VIDIOCGTUNER, &t))
		return;
	if(*f<t.rangelow){
		dprint(LIBVIDEO_SOURCE_CAP, LIBVIDEO_LOG_DEBUG, "TUN: QUIRKS: Tuner frequency %u for tuner %d BELOW min %lu\n", *f, idx, t.rangelow);
		*f = t.rangelow;
		return;
	} else if(*f>t.rangehigh) {
		dprint(LIBVIDEO_SOURCE_CAP, LIBVIDEO_LOG_DEBUG, "TUN: QUIRKS: Tuner frequency %u for tuner %d ABOVE max %lu\n", *f, idx, t.rangehigh);
		*f = t.rangehigh;
		return;
	}
}
...
int set_tuner_freq_v4l1(struct video_device *vdev, int idx, unsigned int f){
	if(-1 == ioctl(vdev->fd, VIDIOCSFREQ, &f)){
		dprint(LIBVIDEO_SOURCE_TUNER, LIBVIDEO_LOG_ERR, "TUN: Failed to set tuner frequency\n");
...
int get_tuner_freq_v4l1(struct video_device *vdev, int idx, unsigned int *f){
	if(-1 == ioctl(vdev->fd, VIDIOCGFREQ, f)){
		dprint(LIBVIDEO_SOURCE_TUNER, LIBVIDEO_LOG_ERR, "TUN: Failed to get tuner frequency\n");
		return LIBVIDEO_ERR_IOCTL;
	}
...
int get_rssi_afc_v4l1(struct video_device *vdev, int idx, int *r, int *a){
	...
	t.tuner = 0;
	if(-1 == ioctl (vdev->fd, VIDIOCGTUNER, &t)){
		dprint(LIBVIDEO_SOURCE_TUNER, LIBVIDEO_LOG_ERR, "TUN: Failed to get tuner info\n");
		return LIBVIDEO_ERR_IOCTL;
	}
	dprint(LIBVIDEO_SOURCE_TUNER, LIBVIDEO_LOG_DEBUG, "TUN: Got RSSI %d & AFC 0\n", t.signal);
	*r = t.signal;
	*a = 0;
	return 0;
}
