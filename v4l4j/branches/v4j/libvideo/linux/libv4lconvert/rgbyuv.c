...
#define YUV2R(y, u, v) CLIP((y) + ((((v)-128)*1436) >> 10))
#define YUV2G(y, u, v) CLIP((y) - ((((u)-128)*352 + ((v)-128)*731) >> 10))
#define YUV2B(y, u, v) CLIP((y) + ((((u)-128)*1814) >> 10))

/**
 * Convert YUV 4:2:0 to 24 bit bgr.
 * @param src YUV 4:2:0 source buffer
 * @param dest bgr24 destination buffer
 * @param width width of frame
 * @param height height of frame
 * @param yvu (boolean) whether the input is YVU 4:2:0
 */
void v4lconvert_yuv420_to_bgr24(const u8 *src, u8 *dest, u32 width, u32 height, int yvu) {
	...
	for (i = 0; i < height; i++) {
		for (j = 0; j < width; j += 2) {
#if CONVERT_FAST /* fast slightly less accurate multiplication free code */
...
void v4lconvert_yuv420_to_rgb24(const u8 *src, u8 *dest, int width, int height, int yvu) {
	int i,j;
...
void v4lconvert_yuyv_to_bgr24(const u8 *src, u8 *dest, int width, int height) {
	int j;
...
#if ARM_SIMD
	int[4] uBuf, vBuf;
	int32x4_t smd128 = vdup_n_u16(128);
	while (--height >= 0) {
		for (j = 0; j < width; j += 2) {
			//load 4 u/v values
			uBuf[0] = src[1];
			...
			uBuf[3] = src[13];
			vBuf[3] = src[15];
			int32x4_t u = vld1q_u32(tmpBuf);
			int32x4_t v = vld1q_u32(tmpBuf);
			
			int32x4x4 y1 = vld4q_dup_s8(src[1]);
		}
	}
#else
	while (--height >= 0) {
		for (j = 0; j < width; j += 2) {
			...
			int v = src[3];
			int u1 = (((u - 128) << 7) + (u - 128)) >> 6;
			int v1 = ((v - 128) << 1) + (v - 128);
			int rg = (((u - 128) << 1) + (u - 128) + (v1 << 1)) >> 3;
			v1 = v1 >> 1;
			
			*dest++ = CLIP(src[0] + v1);//R1
			*dest++ = CLIP(src[0] - rg);//G1
			*dest++ = CLIP(src[0] + u1);//B1
			
			*dest++ = CLIP(src[2] + v1);//R2
			*dest++ = CLIP(src[2] - rg);//G2
			*dest++ = CLIP(src[2] + u1);//B2
			src += 4;
		}
	}
#endif
...
void v4lconvert_yuyv_to_yuv420(const u8 *src, u8 *dest,
...
void v4lconvert_yvyu_to_bgr24(const u8 *src, u8 *dest, int width, int height) {
	int j;
...
/**
 * Convert YVYU to 24 bit RGB.
 */
void v4lconvert_yvyu_to_rgb24(const u8 *src, u8 *dest, int width, int height) {
	int j;
...
void v4lconvert_uyvy_to_bgr24(const u8 *src, u8 *dest, int width, int height) {
	int j;
...
void v4lconvert_uyvy_to_rgb24(const u8 *src, u8 *dest, int width, int height) {
	int j;
...
void v4lconvert_uyvy_to_yuv420(const u8 *src, u8 *dest, int width, int height, int yvu) {
	int i, j;
	const u8 *src1;
	u8 *udest, *vdest;
...
void v4lconvert_swap_rgb(const u8 *src, u8 *dst, int width, int height) {
	int i;
...
void v4lconvert_rgb565_to_rgb24(const u8 *src, u8 *dest, int width, int height) {
	int j;
	while (--height >= 0) {
		for (j = 0; j < width; j++) {
...
void v4lconvert_rgb565_to_bgr24(const u8 *src, u8 *dest, int width, int height) {
	int j;
	while (--height >= 0) {
		for (j = 0; j < width; j++) {
			unsigned short tmp = *(unsigned short *)src;
